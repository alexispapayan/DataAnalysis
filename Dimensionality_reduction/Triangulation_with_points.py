...
def compute_minimum_quality_triangle(triangle,polygon=None):
    ...
    polygon_with_point=np.vstack([polygon,barycenter])
    ...
    for point in triangle: 
        for index,point_in_polygon in enumerate(polygon_with_point):
    ...
    if p2!= polygon_with_point.shape[0]-1:
    ...
    if contains_points(triangle,polygon_with_point):
        return 0
    ...
    shape=dict(holes=hole,vertices=polygon_with_point,segments=contour_connectivity)
    ...
    try:   
        for triangle_index in t['triangles']:
            triangles_in_mesh.append(polygon_with_point[np.asarray([triangle_index])])
    except :
...
def quality_matrix(polygon,compute_minimum=True ,normalize=False):
    ...
    barycenter_point=polygon.sum(0)/polygon.shape[0]
    
    contour_connectivity=np.array(list(tuple(i) for i in get_contour_edges(polygon)))
    
    polygon_with_point=np.vstack([polygon,barycenter_point])
    
    
    quality_matrix=np.zeros([contour_connectivity.shape[0],polygon_with_point.shape[0]])
    #area_matrix=np.zeros([contour_connectivity.shape[0],polygon.shape[0]])
    normals_matrix=np.zeros([contour_connectivity.shape[0],polygon_with_point.shape[0]])

    list_of_triangles=[]
    
    for index,edge in enumerate(contour_connectivity):
        # Not omitting non triangles because either way their quality is zero
        triangles_to_edge_indices=[[*edge,i] for i in range(polygon_with_point.shape[0]) ]
        ...
        triangles=polygon_with_point[triangles_to_edge_indices]
...
def triangulate(polygon,ordered_quality_matrix,recursive=True):
    ...
    triangulated={'segment_markers': np.ones([polygon.shape[0]+1]), 'segments':np.array(get_contour_edges(polygon)), 'triangles': np.array(list( list(i) for i in set_elements)),
                  'vertex_markers': np.ones([polygon.shape[0]+1]), 'vertices':np.vstack([ polygon,polygon.sum(0)/polygon.shape[0]])}
    ...
    set_open_vertices.add(polygon.shape[0])
    ...
    if recursive:
        ...
        for sub_polygon_indices in sub_polygon_list:
            if len(sub_polygon_indices)>=3:
                ...
                polygon_copy=np.vstack([polygon,polygon.sum(0)/polygon.shape[0]])
                ...
                Triangulation.triangulate(sub_polygon,sub_order_matrix,recursive=True)
...
def order_quality_matrix(_quality_matrix,_polygon, check_for_equal=True):
    ...
    quality_board.resize(len(get_contour_edges(_polygon)),len(_polygon)+1,2)
...
def check_ordered_matrix(_order_matrix,polygon):
    ...
    listing=np.empty([len(checked_matrix),len(checked_matrix)+1],dtype=np.float32)
...
#                pdb.set_trace()
                ...
                for index,j in enumerate(non_zero_list):
                    if tag==j:
                        ...
                        triangle=polygon_with_point[triangle_indices]
